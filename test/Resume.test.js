...
describe('Resume', () => {
  ...
  it('successfully uploads a person\'s document', async () => {
    ...
    await resume.methods.addDocument1(title, newIpfsHash.digest, newIpfsHash.hashFunction, newIpfsHash.size).send({
      from: accounts[1], gas: 1000000
    });
    const ipfsHash = await resume.methods.getDocument1(accounts[1]).call();
    assert.equal(multihash.getMultihashFromContractResponse(ipfsHash), originalMultihash);
    assert.equal(web3.utils.hexToUtf8(ipfsHash.title), originalTitle);

    await resume.methods.deleteDocument1(accounts[1]).send({ from: accounts[1], gas: 1000000 });
    const ipfsHashAfterDelete = await resume.methods.getDocument1(accounts[1]).call();
  ...
  it('successfully uploads a person\'s document 2', async () => {
    const originalMultihash = 'QmT6ssjTDE9neaKqBDXGhfFdVcCuh5661iQC7RwPw3RNGj';
    const newIpfsHash = multihash.getBytes32FromMultihash(originalMultihash);
    const originalTitle = 'My Document V1';
    const title = web3.utils.utf8ToHex(originalTitle);
    await resume.methods.addDocument2(title, newIpfsHash.digest, newIpfsHash.hashFunction, newIpfsHash.size).send({
      from: accounts[1], gas: 1000000
    });
    const ipfsHash = await resume.methods.getDocument2(accounts[1]).call();
    assert.equal(multihash.getMultihashFromContractResponse(ipfsHash), originalMultihash);
    assert.equal(web3.utils.hexToUtf8(ipfsHash.title), originalTitle);

    await resume.methods.deleteDocument2(accounts[1]).send({ from: accounts[1], gas: 1000000 });
    const ipfsHashAfterDelete = await resume.methods.getDocument2(accounts[1]).call();
    assert.notEqual(multihash.getMultihashFromContractResponse(ipfsHashAfterDelete), originalMultihash);
    assert.notEqual(web3.utils.hexToUtf8(ipfsHashAfterDelete.title), originalTitle);
  });
  ...
  it('successfully uploads a person\'s document 3', async () => {
    ...
    await resume.methods.deleteDocument3(accounts[1]).send({ from: accounts[1], gas: 1000000 });
    const ipfsHashAfterDelete = await resume.methods.getDocument3(accounts[1]).call();
    assert.notEqual(multihash.getMultihashFromContractResponse(ipfsHashAfterDelete), originalMultihash);
    assert.notEqual(web3.utils.hexToUtf8(ipfsHashAfterDelete.title), originalTitle);
  });

  it('successfully uploads a person\'s document 4', async () => {
    const originalMultihash = 'QmT6ssjTDE9neaKqBDXGhfFdVcCuh5661iQC7RwPw3RNGj';
    const newIpfsHash = multihash.getBytes32FromMultihash(originalMultihash);
    const originalTitle = 'My Document V1';
    const title = web3.utils.utf8ToHex(originalTitle);
    await resume.methods.addDocument4(title, newIpfsHash.digest, newIpfsHash.hashFunction, newIpfsHash.size).send({
      from: accounts[1]
    });
    const ipfsHash = await resume.methods.getDocument4(accounts[1]).call();
    assert.equal(multihash.getMultihashFromContractResponse(ipfsHash), originalMultihash);
    assert.equal(web3.utils.hexToUtf8(ipfsHash.title), originalTitle);

    await resume.methods.deleteDocument4(accounts[1]).send({ from: accounts[1], gas: 1000000 });
    const ipfsHashAfterDelete = await resume.methods.getDocument4(accounts[1]).call();
    assert.notEqual(multihash.getMultihashFromContractResponse(ipfsHashAfterDelete), originalMultihash);
    assert.notEqual(web3.utils.hexToUtf8(ipfsHashAfterDelete.title), originalTitle);
  });

  it('successfully uploads a person\'s document 5', async () => {
    const originalMultihash = 'QmT6ssjTDE9neaKqBDXGhfFdVcCuh5661iQC7RwPw3RNGj';
    const newIpfsHash = multihash.getBytes32FromMultihash(originalMultihash);
    const originalTitle = 'My Document V1';
    const title = web3.utils.utf8ToHex(originalTitle);
    await resume.methods.addDocument5(title, newIpfsHash.digest, newIpfsHash.hashFunction, newIpfsHash.size).send({
      from: accounts[1]
    });
    const ipfsHash = await resume.methods.getDocument5(accounts[1]).call();
    assert.equal(multihash.getMultihashFromContractResponse(ipfsHash), originalMultihash);
    assert.equal(web3.utils.hexToUtf8(ipfsHash.title), originalTitle);

    await resume.methods.deleteDocument5(accounts[1]).send({ from: accounts[1], gas: 1000000 });
    const ipfsHashAfterDelete = await resume.methods.getDocument5(accounts[1]).call();
    assert.notEqual(multihash.getMultihashFromContractResponse(ipfsHashAfterDelete), originalMultihash);
    assert.notEqual(web3.utils.hexToUtf8(ipfsHashAfterDelete.title), originalTitle);
  });
